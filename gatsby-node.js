...
const createPages = async ({ graphql, actions }) => {
  ...
  posts.forEach((post, i) => {
    ...
    if (post.node.frontmatter.course) {
      post.node.frontmatter.course.forEach((tag) => {
        courseet.add(tag)
      })
    }

    if (post.node.frontmatter.noteType) {
      post.node.frontmatter.noteType.forEach((noteType) => {
        noteTypeSet.add(noteType)
      })
    }
